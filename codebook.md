...
|----------|------|-------------|-------|
| `id` | character | Document identifier | CELEX number for Eur-Lex docs, ipnum for press releases |
| `corpus` | character | Source corpus | EurLex = Eur-Lex policy documents, Press = Commission press releases |
| `type` | character | Document type | Coded types extracted from document titles: Commission Staff Working Document, Communication, Legislative Decision, Legislative Proposal, Report, Press Release, Other | 
| `year` | integer | Publication year | Range: 1985-2020 |
| `return_focus` | character | Boolean indicator (TRUE/FALSE) of whether a document focuses specifically on return policy| TRUE: 686, FALSE: 2663|
| `date` | character | Publication date | Range: 09.04.1985 - 23.12.2020  |
| `title` | character | Document title | Note: concatenated for duplicate texts with variant metadata |
| `text` | character | Document text | Document text (cleaning steps see below) |
| `word_count` | integer | Number of words | Counted as whitespace-separated tokens |

## Data Cleaning

The following artifacts were systematically removed from document texts:
...
- Filtered by Eurovoc descriptors and title keywords. For used desxriptors see below
...
- **Total documents**: 3349
- **Time range**: 1985 - 2020
- **Eur-Lex documents**: 1499
- **Press releases**: 1850
- **Average word count**: 4249 words
